...
bool checkSudoku(int sudoku[][3], int row, int col) {
    int i, j, k, l;
    int count = 0;
    int* values = malloc(row * col * sizeof(int));  // Dynamically allocate memory for values array

    printf("\n");
    for (i = 0; i < row; i++) {
        for (j = 0; j < col; j++) {
            for (k = i; k < row; k++) {  // Start from i to avoid checking duplicate pairs
                for (l = (k == i) ? j + 1 : 0; l < col; l++) {  // Adjusted the loop conditions
