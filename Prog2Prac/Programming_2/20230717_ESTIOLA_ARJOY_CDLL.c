...
void makeCDLL(LIST* graduating_studs);
...
int main()
{
    ...
    graduating = retrieveGraduatingDCISMStuds(&dcism_studs, &graduating_studs);
	
	makeCDLL(graduating_studs);
...
void enroll(LIST** head)
{
    ...
    if (newNode != NULL) {
        ...
        printf("Last Name: ");
        scanf("%s", newNode->data.name.LName);
        printf("First Name: ");
        scanf("%s", newNode->data.name.FName);
    ...
    while (current != NULL) {
        if (current->data.year == 4 && ((strcmp(current->data.course, "BSIT") == 0) || (strcmp(current->data.course, "BSCS") == 0) || (strcmp(current->data.course, "BSIS") == 0))) {
            ...
            if (*graduating_studs == NULL) {
                *graduating_studs = current;
            } else {
                (*graduating_studs)->prev = current;
                current->next = *graduating_studs;
                *graduating_studs = current;
            }
...
void freeLists(LIST* dcism_studs, LIST* graduating_studs)
{
    LIST* current = dcism_studs;
    LIST* next;
